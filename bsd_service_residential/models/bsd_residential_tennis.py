...
class BsdTennis(models.Model):
    _name = 'bsd.tennis'
    _description = 'Sân tennis'

    name = fields.Char(string='Tên sân')


class BsdResidentialTennis(models.Model):
    _name = 'bsd.residential.tennis'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin']
    _description = "Đăng ký dịch vụ Tennis"
    ...
    bsd_tennis_id = fields.Many2one('bsd.tennis', string="Sân", required=True)
    ...
    def action_card(self):
        ...
        if qrcode and base64:
            str_qrcode = url + '/service/tennis/' + str(self.id) + '?access_token=' + self.access_token
            qr = qrcode.QRCode(
                    version=1,
    ...
    def action_payment(self):
        pay_id = self.env['account.payment'].create({
            ...
            'communication': 'Đặt sân tennis ' + self.name ,
    ...
    def create(self, vals):
        if vals.get('name', 'New') == 'New':
            vals['name'] = self.env['ir.sequence'].next_by_code('bsd.residential.tennis') or '/'
        return super(BsdResidentialTennis, self).create(vals)
