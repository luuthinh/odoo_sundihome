...
class BsdResidentialSwim(models.Model):
    _name = 'bsd.residential.swim'
    _inherit = ['portal.mixin', 'mail.thread', 'mail.activity.mixin', 'utm.mixin', 'image.mixin']
    _description = 'Hội viên hồ bơi'
    ...
    bsd_line_ids = fields.One2many('bsd.residential.swim.line', 'bsd_residential_swim_id', string="Thời gian đăng ký",
                                   states={'draft': [('readonly', False)],
                                           'active': [('readonly', True)],
                                           'deactivate': [('readonly', True)],
                                           'done': [('readonly', True)],
    ...
    bsd_residential_swim_pause_ids = fields.One2many('bsd.residential.swim.pause', 'bsd_residential_swim_id',
                                                    string="Bảo lưu")
    ...
    def action_card(self):
        ...
        if qrcode and base64:
            str_qrcode = url + '/service/swim/' + str(self.id) + '?access_token=' + self.access_token
            qr = qrcode.QRCode(
                    version=1,
    ...
    def action_pause(self):
        action = self.env.ref('bsd_service_residential.bsd_wizard_swim_pause_action').read()[0]
        return action

    def action_run(self):
        action = self.env.ref('bsd_service_residential.bsd_wizard_swim_run_action').read()[0]
        return action
    ...
    def create(self, vals):
        if vals.get('bsd_sequence', 'New') == 'New':
            vals['bsd_sequence'] = self.env['ir.sequence'].next_by_code('bsd.residential.swim') or '/'
        return super(BsdResidentialSwim, self).create(vals)


class BsdResidentialSwimLine(models.Model):
    _name = "bsd.residential.swim.line"
    ...
    bsd_residential_swim_id = fields.Many2one('bsd.residential.swim', string="Hội viên")
...
class BsdResidentialGymPause(models.Model):
    _name = "bsd.residential.swim.pause"
    _description = "Thời gian bảo lưu của hội viên hồ bơi"
    _rec_name = "bsd_residential_swim_id"
